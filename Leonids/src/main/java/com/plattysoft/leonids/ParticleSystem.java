...
public class ParticleSystem {
    ...
    private int mExiterXMin;
    private int mExiterXMax;
    private int mExiterYMin;
    private int mExiterYMax;
    ...
    private void startEmiting(int particlesPerSecond) {
        ...
        mExitingTime = -1; // Meaning infinite
    ...
    private void configureEmiter(int emitterX, int emitterY) {
        ...
        mExiterXMin = emitterX - mParentLocation[0];
        mExiterXMax = mExiterXMin;
        mExiterYMin = emitterY - mParentLocation[1];
        mExiterYMax = mExiterYMin;
    }
    ...
    private void configureEmiter(int emitterX, int emitterMinY, int emitterMaxY) {
        ...
        mExiterXMin = emitterX - mParentLocation[0];
        mExiterXMax = mExiterXMin;
        mExiterYMin = emitterMinY;
        mExiterYMax = emitterMaxY;
    }
    ...
    private void configureEmiter(View emiter, int gravity) {
        ...
        if (hasGravity(gravity, Gravity.LEFT)) {
            mExiterXMin = location[0] - mParentLocation[0];
            mExiterXMax = mExiterXMin;
        } else if (hasGravity(gravity, Gravity.RIGHT)) {
            mExiterXMin = location[0] + emiter.getWidth() - mParentLocation[0];
            mExiterXMax = mExiterXMin;
        } else if (hasGravity(gravity, Gravity.CENTER_HORIZONTAL)) {
            mExiterXMin = location[0] + emiter.getWidth() / 2 - mParentLocation[0];
            mExiterXMax = mExiterXMin;
        } else {
            // All the range
            mExiterXMin = location[0] - mParentLocation[0];
            mExiterXMax = location[0] + emiter.getWidth() - mParentLocation[0];
        }

        // Now, vertical gravity and range
        if (hasGravity(gravity, Gravity.TOP)) {
            mExiterYMin = location[1] - mParentLocation[1];
            mExiterYMax = mExiterYMin;
        } else if (hasGravity(gravity, Gravity.BOTTOM)) {
            mExiterYMin = location[1] + emiter.getHeight() - mParentLocation[1];
            mExiterYMax = mExiterYMin;
        } else if (hasGravity(gravity, Gravity.CENTER_VERTICAL)) {
            mExiterYMin = location[1] + emiter.getHeight() / 2 - mParentLocation[1];
            mExiterYMax = mExiterYMin;
        } else {
            // All the range
            mExiterYMin = location[1] - mParentLocation[1];
            mExiterYMax = location[1] + emiter.getHeight() - mParentLocation[1];
        }
    }
    ...
    private void activateParticle(long delay) {
        ...
        int particleX = getFromRange(mExiterXMin, mExiterXMax);
        int particleY = getFromRange(mExiterYMin, mExiterYMax);
    ...
    private void onUpdate(long miliseconds) {
        while (((mExitingTime > 0 && miliseconds < mExitingTime) || mExitingTime == -1) && // This point should emit
